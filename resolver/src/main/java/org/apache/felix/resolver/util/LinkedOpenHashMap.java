...
public class LinkedOpenHashMap<K, V>  implements Serializable, Cloneable, SortedMap<K, V> {
    ...
    public LinkedOpenHashMap(int expected, float f) {
        this.first = -1;
        this.last = -1;
        if (f > 0.0F && f <= 1.0F) {
            if (expected < 0) {
    ...
    public LinkedOpenHashMap(int expected) {
        this(expected, 0.75F);
    }
    ...
    public LinkedOpenHashMap(Map<? extends K, ? extends V> m, float f) {
        this(m.size(), f);
        this.putAll(m);
    }

    public LinkedOpenHashMap(Map<? extends K, ? extends V> m) {
        this(m, 0.75F);
    }

    public LinkedOpenHashMap(K[] k, V[] v, float f) {
        this(k.length, f);
    ...
    public LinkedOpenHashMap(K[] k, V[] v) {
        this(k, v, 0.75F);
    }
    ...
    */
    ...
    public Iterable<Map.Entry<K, V>> fast() {
        if (this.fast == null) {
            this.fast = new Iterable<Entry<K, V>>() {
    ...
    public SortedSet<K> keySet() {
        if (this.keys == null) {
            this.keys = new LinkedOpenHashMap.KeySet();
        }
    ...
    public void concat() {
        rehash(arraySize(this.size, 1.0f));
    }
    ...
    public boolean trim(int n) {
        int l = nextPowerOfTwo((int) Math.ceil((double) ((float) n / this.f)));
        if (this.n <= l) {
            return true;
        } else {
            try {
    ...
    protected void rehash(int newN) {
        ...
        for (int j = this.size; j-- != 0; prev = t) {
            ...
            if (key[i] == null) {
                ...
                for (pos = mix(key[i].hashCode()) & mask; newKey[pos] != null; pos = pos + 1 & mask) {
                    ;
                }
    ...
    @SuppressWarnings("unchecked")
    public LinkedOpenHashMap<K, V> clone() {
    ...
    private void writeObject(ObjectOutputStream s) throws IOException {
        Object[] key = this.key;
        Object[] value = this.value;
        LinkedOpenHashMap.MapIterator i = new LinkedOpenHashMap.MapIterator(null);
    ...
    private final class KeySet extends AbstractObjectSet<K> implements SortedSet<K> {
        ...
        public boolean remove(Object k) {
            int oldSize = size;
            LinkedOpenHashMap.this.remove(k);
            return size != oldSize;
        }
    ...
    private class FastEntryIterator extends MapIterator implements Iterator<Entry<K, V>> {
        ...
        public LinkedOpenHashMap.MapEntry next() {
            this.entry.index = this.nextEntry();
            return this.entry;
        }

        public LinkedOpenHashMap.MapEntry previous() {
    ...
    private class EntryIterator extends MapIterator implements Iterator<Entry<K, V>> {
        private LinkedOpenHashMap.MapEntry entry;
